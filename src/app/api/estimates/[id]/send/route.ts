...
import { sendEstimate } from "@/lib/estimates/send";
import { EstimateSendSchema } from "@/lib/validation/estimate";
import { checkRateLimit } from "@/lib/security/rate-limit";
import { recordAuditEvent } from "@/lib/security/audit";

// POST /api/estimates/[id]/send — manual send (preview by default; live requires
// ESTIMATE_EMAIL_MODE=live, the external-action approval gate).
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = EstimateSendSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    await recordAuditEvent({
      tenantId,
      userId,
      actionType: result.delivered ? "estimate.sent" : "estimate.sent",
      description: `Sent estimate ${id} (${result.previewMode ? "preview" : result.delivered ? "delivered" : "queued"})`,
      entityType: "estimate",
      entityId: id,
    ...
    return NextResponse.json({
      data: {
        ...
        testOverride: result.testOverride,
        // The public URL is returned so the operator can copy/paste in preview mode.
        publicUrl: result.publicUrl,
      },
    });
  } catch (err) {
    console.error("[api] POST /api/estimates/[id]/send:", err);
...
function sendErrorMessage(reason: string): string {
  switch (reason) {
    case "not_found": return "Estimate not found";
    case "stale_version": return "This estimate changed since you loaded it. Reload and try again.";
    case "invalid_state": return "This estimate cannot be sent from its current status";
    ...
    default: return "Failed to send estimate";
  }
}
