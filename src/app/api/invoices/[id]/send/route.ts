...
import { InvoiceSendSchema } from "@/lib/validation/invoice";
import { checkRateLimit } from "@/lib/security/rate-limit";
import { recordAuditEvent } from "@/lib/security/audit";

// POST /api/invoices/[id]/send — manual send (preview by default, same mailer
// gate as estimates/change orders via canSendEstimateEmail).
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await recordAuditEvent({
      ...
      description: `Sent invoice ${id} (${result.previewMode ? "preview" : result.delivered ? "delivered" : "queued"})`,
      entityType: "invoice",
      entityId: id,
    ...
    console.error("[api] POST /api/invoices/[id]/send:", err);
...
function sendErrorMessage(reason: string): string {
  switch (reason) {
    ...
    case "stale_version": return "This invoice changed since you loaded it. Reload and try again.";
    case "invalid_state": return "This invoice cannot be sent from its current status";
    ...
    default: return "Failed to send invoice";
  }
}
