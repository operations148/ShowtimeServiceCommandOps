...
| 6 — Invoices, Stripe Connect payments, ledger, reconciliation | ✅ **merged + deployed** 2026-07-14 (Stripe keys deferred) | `feat/serviceops-phase-6-invoices-payments` → master | 12-state invoice machine, immutable payment ledger (ADR-0012), Stripe Connect Express/direct-charge + server-owned amounts (ADR-0013), webhook rewrite (verify + terminal/transient split), public pay page `/invoice/[token]`, admin invoices UI `/dashboard/invoices`, reconciliation cron+admin, `specs/invoices-and-payments.md`, `database-blueprint/payments.md`, `docs/operations/stripe-runbook.md`, `qa/payments-test-plan.md`, `memory/phase-6-invoices-payments.md`. Migration `20260714000001` **applied to prod**. **Remaining**: set `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` (test keys) + create the Stripe webhook endpoint to enable card payment. |
| 7 — Secure tenant-branded customer portal | ✅ **merged + deployed** 2026-07-16 | `feat/serviceops-phase-7-customer-portal` → master | Separate `portal_customers` identity + passwordless magic-link (hashed/one-time/scanner-safe) + DB-backed revocable sessions re-validated per request, property-scoped authz reusing existing accept/decline/pay verbs, customer UI `/portal/*`, admin `/dashboard/portal-users` gated by new `canManagePortalUsers`, no-store cache policy + PWA manifest, ADR-0014, `specs/customer-portal.md`, `database-blueprint/customer-portal.md`, `docs/security/portal-security.md`, `qa/customer-portal-test-plan.md`, `memory/phase-7-customer-portal.md`. Migration `20260715000001` **applied to prod**. Live at `serviceops-ghl-workorders-chi.vercel.app`. Customer email stays preview-gated until client approves live sends. |
| 8 — Technician PWA & offline resilience | ✅ code-complete 2026-07-17 (branch, **not merged/deployed**, no migration) | `feat/serviceops-phase-8-technician-pwa` | Resilient online-first (NOT a full replay queue — ADR-0015): SW `runtimeCaching` (NetworkFirst) scoped to `/tech` + `GET /api/visits`, localStorage checklist/notes drafts, IndexedDB outbox for the visit PATCH (auto-flush on reconnect), idempotent photo dedup via `client_photo_id`, `NEXT_PUBLIC_OFFLINE_SYNC_ENABLED` kill-switch. `src/lib/offline/*` + `JobDetail` wiring + `OfflineBanner`. ADR-0015, `specs/technician-pwa-offline.md`, `qa/technician-offline-test-plan.md`, `memory/phase-8-technician-pwa.md`. **To deploy**: merge→master, `vercel --prod` (no migration, no new secret). |

| 9 — Time, mileage, expenses, job costing | ✅ **merged + deployed** 2026-07-17 (migration `20260717000001` applied) | `feat/serviceops-phase-9-job-costing` → master | Wires the Phase 5 `work_orders.actual_cost_cents` stub at last. Frozen rate snapshots (a raise never rewrites historical margin), `actual_cost_cents` DERIVED via full recompute never incremented, cost-blind technicians (log yes / see cost never) via a server-side allowlist serializer + 3 new role flags (`canLogJobCosts`/`canViewJobCosting`/`canManageJobCosting`), margin derived with `margin_percent` null-vs-0. `time_entries`/`mileage_entries`/`job_expenses` + rate columns. ADR-0016, `specs/job-costing.md`, `database-blueprint/job-costing.md`, `qa/job-costing-test-plan.md`, `memory/phase-9-job-costing.md`. **To deploy**: merge→master, apply migration `20260717000001`, set tenant rates + technician `hourly_cost_cents` (otherwise everything correctly costs $0), `vercel --prod`. |

| 10 — Reporting platform, admin, white-label | ✅ **merged + deployed** 2026-07-17 (migration `20260717000003` applied; platform-admin flag left OFF) | `feat/serviceops-phase-10-reporting-admin` → master | Financial reporting (`/dashboard/reports/financial`, `GET /api/reports/financial`) — first surface joining Phase 6 revenue + Phase 9 cost: revenue as invoiced/collected/outstanding/written-off, cost breakdown, gross profit + margin (null-vs-0), AR aging, caveats rendered as data. Owner-only (`canViewFinancialReports`). Platform admin (`/dashboard/platform`, `/api/platform/tenants`) for `canManageTenants` — double-gated (`NEXT_PUBLIC_PLATFORM_ADMIN_ENABLED` kill-switch default OFF → 404, + platform_owner), metadata-only (no cross-tenant PII), activate/suspend audited. White-label: `ghl_api_token_encrypted` deprecated via `COMMENT` (migration `20260717000003`) + `@deprecated` type, NOT built (ADR-0017 — one shared GHL token is the reality). specs/reporting-platform-admin.md, qa/reporting-platform-admin-test-plan.md, memory/phase-10-reporting-platform-admin.md. **To deploy**: merge→master, apply `20260717000003` (comment-only), `vercel --prod`; leave `NEXT_PUBLIC_PLATFORM_ADMIN_ENABLED` unset until a 2nd tenant. |

| 11 — Production readiness, security, QA | ✅ **merged + deployed** 2026-07-18 — **CI GREEN on master for the first time ever** (run for `750f407`: success; previously 26/26 red). Dependabot PRs now flowing; its Next-16 bump PR fails CI, confirming both the pipeline's discrimination and the decision to schedule that upgrade. | `feat/serviceops-phase-11-production-readiness` → master | **The go/no-go document: `docs/launch-readiness-report.md`** + fully reconciled `qa/launch-readiness-checklist.md` (old one was 2026-05-14, pre-expansion). Found+fixed: CI secret-scan step false-failed EVERY master push (inverted empty-diff logic; now scans tracked files). Removed `@types/next-pwa` (phantom vulnerable `next@13`, audit 10→9). Audit analyzed per-finding: Next 15.5.20 NOT affected by the middleware-bypass CVE; residuals = scheduled upgrades (Next 16, Auth.js v5, next-pwa fork), not blockers. Verified live: headers, RLS complete, crons fail-closed, webhook 401, rate limiter E2E, 32/32 migrations, 413 tests. Launch gate = config (Stripe keys, portal email, costing rates, GHL webhook registration) + human QA (real phone, test payment, real GHL payload, pilot customer). |

| 12 — Completion→GHL review webhook + technician location map | ✅ **merged + deployed** 2026-07-19 (migration `20260719000001` applied; CI green; routes verified live) | `feat/serviceops-phase-12-completion-webhook-map` → master | **Bug fix**: tech-mobile job completion never notified GHL (only the admin PATCH route did) — most real completions silently skipped the client's review workflow. Now both paths fire `syncCompletionToGhl`, which ALSO POSTs a completion payload to a tenant-set GHL **Inbound Webhook** URL (`tenants.ghl_completion_webhook_url`, Settings→Company) to trigger the Google-review workflow; durable via the GHL outbox (`job_type: completion_webhook`). **Map**: Leaflet/OSM (no Google billing) on WO detail — property pin + assigned tech LAST-KNOWN location, foreground-only (PWA-honest), freshness-labeled (live/recent/stale), dispatch-gated (`canViewSchedule`). `technician_locations` = 1 row/tech, no history. Nominatim geocode cached on properties. Kill-switch `NEXT_PUBLIC_TECH_LOCATION_ENABLED`. ADR-0018, `specs/completion-webhook-and-map.md`. Migration `20260719000001`. Added `leaflet` dep. **To deploy**: merge→master, apply migration, client pastes GHL webhook URL, `vercel --prod`. |

**RLS backfill hotfix (2026-07-17, applied to prod as `20260717000002`)**: Supabase's advisor flagged 5 tables (`invoice_line_items`, `rate_limits`, `recurring_schedules`, `webhook_events`, `work_order_status_history`) with RLS disabled — the PUBLIC anon key could read `rate_limits` live (login-attempt emails) and could have deleted them (resetting brute-force protection). Fixed: tenant tables got the standard policy pattern; internal tables (no tenant_id) got RLS-with-no-policies + REVOKE from anon/authenticated. Verified after: anon PostgREST access denied on all 5, login/rate-limiter/app unaffected (the app is service-role only). Same day: `EditTechnicianPanel` gained the owner-only "Labor Cost per Hour" input (per-tech burdened rate, `/api/technicians/[id]/rate` GET+PATCH). See `docs/security/security-controls.md` addendum.

**Migration-history hotfixes applied to production this session** (both hand-added columns never captured in tracked migrations, breaking a from-migrations DB): `users.avatar_url` (`20260714000002` — was breaking login) and `tenants.logo_url` (`20260714000003` — breaks company settings + document-send branding). The production DB (`yyjbfjnpmjcraehecbvi`) now has Phases 0–6 applied + these two hotfixes; Phase 7's `20260715000001` is NOT yet applied.
