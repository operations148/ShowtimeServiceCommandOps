...
| 9 — Time, mileage, expenses, job costing | ✅ code-complete 2026-07-17 (branch, **not merged/deployed**) | `feat/serviceops-phase-9-job-costing` | Wires the Phase 5 `work_orders.actual_cost_cents` stub at last. Frozen rate snapshots (a raise never rewrites historical margin), `actual_cost_cents` DERIVED via full recompute never incremented, cost-blind technicians (log yes / see cost never) via a server-side allowlist serializer + 3 new role flags (`canLogJobCosts`/`canViewJobCosting`/`canManageJobCosting`), margin derived with `margin_percent` null-vs-0. `time_entries`/`mileage_entries`/`job_expenses` + rate columns. ADR-0016, `specs/job-costing.md`, `database-blueprint/job-costing.md`, `qa/job-costing-test-plan.md`, `memory/phase-9-job-costing.md`. **To deploy**: merge→master, apply migration `20260717000001`, set tenant rates + technician `hourly_cost_cents` (otherwise everything correctly costs $0), `vercel --prod`. |
