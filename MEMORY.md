...
| 15 | Estimate → Invoice → Payment System | 🔄 In Progress — migration (20260613000001_invoices.sql) applied ✅, src/types/invoice.ts ✅, src/lib/validation/invoice.ts ✅, src/lib/db/queries/invoices.ts ✅, estimate locking in PATCH /api/work-orders/[id] ✅, src/lib/invoicing/create-invoice-from-estimate.ts ✅ (idempotent, deposit_due, INV-XXXX). **BLOCKED**: invoices table missing GRANT to service_role — apply migration 20260617000001 in Supabase SQL Editor. Remaining: fix grant, public /estimate/[token] page, Stripe Connect deposit flow. |
| 16 | ContactCreate/ContactUpdate Webhooks | ⏳ Not Started (handler file `upsert-property-from-ghl.ts` exists, not wired to dispatch) |
| 17 | AppointmentBooked Webhook | ⏳ Not Started (handler file `create-work-order-from-appointment.ts` exists, not wired) |
| 18 | GHL Retry Queue Persistence | ⏳ Not Started (currently in-memory only, needs DB table backing) |
